...
| Repository, GitHub tree, codebase, system design | Architecture workspace: clickable repo tree, service/data-flow diagram, key files, runtime/deploy map |
...
- Explorer-detail pair: compact left navigation with a right-side summary pane.
...
- Directory explorer: render the project structure like a code editor sidebar
  with folder/file icons, nested indentation, selected state, and a side detail
  pane with one-line summaries plus 2-4 useful facts.
...
- Keep the tree compact, like VS Code's Explorer.
